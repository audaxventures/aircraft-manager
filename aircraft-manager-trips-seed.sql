-- Clear these three tables first (safe no-op if already empty) so the
-- inserts below don't depend on any unique constraint existing yet.
DELETE FROM public."TripPassenger";
